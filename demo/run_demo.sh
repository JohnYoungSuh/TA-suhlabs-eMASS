...
if [ ! -d "venv" ]; then
    echo -e "${RED}❌ Virtual environment not found. Please run ./setup_demo.sh first${NC}"
    exit 1
fi
...
echo ""
echo -e "${BLUE}Step 3: Creating Demo Video...${NC}"
echo ""
...
if [ -f "demo_output/emass_demo.webm" ]; then
    ...
    echo "  ffmpeg -i demo_output/emass_demo.webm -c:v libx264 -crf 23 -c:a aac demo_output/emass_demo.mp4"
else
